...
import com.telcobright.party.v2.model.FieldMeta;
...
/**
 * Odoo-backed user repo provider. Used by ancillary endpoints
 * (/v2/health/adapters, /v2/tenants/{id}/entities) — NOT on the auth path
 * (that runs through {@link OdooLoginPolicy} directly).
 *
 * Authentication uses {@code common.authenticate}; the user's own password is
 * then re-used for {@code execute_kw} read calls (Odoo allows res.users
 * self-read with the user's own credentials, so no admin secret is required
 * for basic auth).
 *
 * For administrative read paths (findByLogin / search) AND for schema
 * introspection (the {@link #entities()} call), an admin password is
 * required — configured via {@code odoo.admin-user} + {@code odoo.admin-password}.
 * When those are absent, administrative methods degrade to empty / names-only
 * results.
 */
public final class OdooUserRepoProvider implements UserRepoProvider {
    ...
    public OdooUserRepoProvider(String baseUrl, String db, int timeoutMillis,
    ...
    public AuthResult authenticate(String login, String password) {
        ...
        try {
            ...
            return AuthResult.deny("provider error: " + ae.getMessage());
        }
        if (uid.isEmpty()) {
